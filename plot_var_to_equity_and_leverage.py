...
Output:
- output/figures/var_to_equity/figure5_var99.png
"""
...
# Bank selection
# -----------------------------
BANK_IDS = ["goldmansachs", "morganstanley"]
...
def out_file() -> Path:
    return ROOT / "output" / "figures" / "var_to_equity" / "figure5_var99"  
...
def main() -> None:
    ...
    # Load the balance sheet panel used for the denominator variables.
    ...
    # Load the harmonized 99% VaR series.
    var = pd.read_csv(VAR_FILE)
    var = var[["bank", "date", "var_99_gaussian"]].copy()
    var = var.rename(
        columns={
            "bank": "bank_id",
            "date": "period_end_date",
            "var_99_gaussian": "var99",
        }
    )
    ...
    # Coerce the core variables to numeric types before merging.
    ...
    # Merge balance sheet data and VaR on bank and quarter-end date.
    ...
    # Standardize within each bank relative to the pre-period.
    cols_to_std = ["unit_var", "var_to_equity", "leverage"]

    d = (
        d.groupby("bank_id", group_keys=False)
        .apply(lambda grp: zscore_with_reference(grp, cols_to_std))
        .reset_index(drop=True)
    )

    # Aggregate bank-level standardized series to quarter-level averages.
    g = (
