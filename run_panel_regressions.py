"""Estimate panel regression specifications and export formatted result tables."""
...
# Bank-group definitions used in interaction terms.
MARKET_BANKS  = ["goldmansachs", "morganstanley"]
CUSTODY_BANKS = ["statestreet", "bny"]

# Standard-error specifications.
# clustered: main specification clustered by bank.
# robust: heteroskedasticity-robust specification.
COV_TYPES = {
    "clustered": "clustered",
    "robust":    "robust",
}

# Output filenames by covariance estimator.
OUTPUT_FILES = {
...
# bank_fe: include bank fixed effects.
# time_fe: include time fixed effects.
# banks: optional bank subset; None keeps the full sample.
# date_range: sample window on the form ("YYYY-Qn", "YYYY-Qn").
...
MODELS = {
    ...
    # Additional sample splits and subgroup specifications.
    "M1A_pre_covid": {
        "y":          "d_ln_leverage",
    ...
    # Pre/post-COVID specifications with bank-group interactions.
    "M2A_pre_covid_dummy": {
        "y":          "d_ln_leverage",
        "X":          ["d_ln_unit_var_lag", "d_ln_unit_var_x_market", "d_ln_unit_var_x_custody", "size_lag", "roa_lag"],
        "bank_fe":    True,
...
# Sort the panel and remove duplicate bank-quarter observations.
df = (
    df
...
# Bank-type indicators and sample split indicators.
...
# First differences relative to the previous quarter.
DIFF_VARS = [
    # (source column, new differenced variable)
...
# Lagged variables from the previous quarter.
LAG_VARS = [
...
# Interaction terms used across the regression specifications.
INTERACTIONS = [
    # (new variable, factor 1, factor 2)
...
# Set the panel index required by linearmodels.
...
def run_model(name, spec, cov_type):
    ...
    # Restrict the sample to the requested banks and time window.
    sub = df[[y] + X].copy()
    ...
    # Estimate each model on a complete-case sample.
    sub = sub.dropna()
    if len(sub) < 30:
        return {"name": name, "error": f"Too few observations: {len(sub)}"}

    # Build the fixed-effects part of the specification.
    fe_terms = (["EntityEffects"] if bank_fe else []) + (["TimeEffects"] if time_fe else [])

    # Use PanelOLS when fixed effects are requested; otherwise use PooledOLS.
    ...
    # Select the covariance estimator for the fitted model.
    fit_kwargs = (
...
# Estimate every model under both covariance estimators.
all_results = {
...
# ══════════════════════════════════════════════════════════════════════════════
#  FORMAT AND SAVE OUTPUT TABLES
# ══════════════════════════════════════════════════════════════════════════════

def build_csv(results):
    rows = []

    # Collect the union of all estimated parameters across models.
    all_params = []
    for r in results:
        if "result" in r:
    ...
    # Store one row for coefficients and one row for t-statistics.
    ...
    # Append model metadata below the coefficient panel.
...
# Save one results file per covariance specification.
